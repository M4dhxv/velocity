...
function buildHeuristicProfileFromTranscript(transcript = '') {
  ...
  skillMap.forEach(([needle, label]) => {
    if (text.includes(needle) && !skills.includes(label)) skills.push(label);
  });
  ...
  const interestMap = [
    ...
    ['quick', 'quick tasks'],
    ['flexible', 'flexible gigs'],
  ];

  interestMap.forEach(([needle, label]) => {
    if (text.includes(needle) && !interests.includes(label)) interests.push(label);
  });

  let workType = '';
  if (text.includes('remote')) workType = 'remote';
  else if (text.includes('local')) workType = 'local';
  else if (text.includes('part time')) workType = 'part-time';
  else if (text.includes('full time')) workType = 'full-time';

  let level = '';
  if (text.includes('beginner') || text.includes('junior')) level = 'beginner';
  else if (text.includes('advanced') || text.includes('senior')) level = 'advanced';
...
export async function extractProfileFromTranscript(transcript) {
  ...
  try {
    ...
    if (!response.ok) {
      const err = await response.text();
      console.error(`LLM failed: ${response.status} ${err}`);
      return validateProfile(buildHeuristicProfileFromTranscript(transcript));
    }
    ...
    const hasAnyStructuredContent =
      ...
      Boolean(parsed.work_type) ||
      Boolean(parsed.level) ||
      Boolean(parsed.experience);
    ...
    console.error('LLM exception, using heuristic profile fallback', error);
    return validateProfile(buildHeuristicProfileFromTranscript(transcript));
  }
}
