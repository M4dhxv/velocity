...
function inferFallbackProfileFromTranscript(transcript = '') {
  const text = String(transcript || '').toLowerCase();

  const lexicon = [
    'react', 'javascript', 'typescript', 'node', 'nodejs', 'python', 'java', 'c++',
    'html', 'css', 'sql', 'mongodb', 'postgres', 'figma', 'photoshop', 'seo',
    'sales', 'customer support', 'video editing', 'copywriting', 'qa', 'testing',
    'sdet', 'frontend', 'backend', 'full stack', 'automation', 'api', 'playwright',
  ];

  const skills = lexicon.filter((term) => {
    const escaped = term.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    return new RegExp(`(^|[^a-z0-9])${escaped}([^a-z0-9]|$)`, 'i').test(text);
  });

  const interests = [];
  if (/frontend|react|ui|ux/.test(text)) interests.push('frontend development');
  if (/backend|api|server/.test(text)) interests.push('backend development');
  if (/qa|test|testing|sdet/.test(text)) interests.push('quality assurance');
  if (/design|figma|photoshop/.test(text)) interests.push('design');
  if (/sales|marketing|seo/.test(text)) interests.push('sales and marketing');

  let workType = 'any';
  if (/part[ -]?time/.test(text)) workType = 'part-time';
  else if (/full[ -]?time/.test(text)) workType = 'full-time';
  else if (/remote/.test(text)) workType = 'remote';
  else if (/local|onsite|on-site/.test(text)) workType = 'local';

  let level = 'intermediate';
  if (/beginner|entry|fresher|junior|new to/.test(text)) level = 'beginner';
  else if (/senior|lead|expert|advanced|\b\d+\+?\s*years?/.test(text)) level = 'advanced';
...
export async function extractProfileFromTranscript(transcript, options = {}) {
  ...
  if (!geminiApiKey) {
    const fallback = inferFallbackProfileFromTranscript(transcript);
    if (includeDebug) {
      return {
        profile: fallback,
        model: 'fallback-heuristic',
        context: buildProfileExtractionContext(transcript),
  ...
  for (const model of modelCandidates) {
    try {
      ...
      // For non-404s, try other candidates and fall back heuristically if all fail.
      if (response.status !== 404) continue;
    } catch (e) {
      lastError = e?.message || 'gemini_fetch_error';
      console.error(`[extractProfileFromTranscript] Model ${model} threw:`, lastError);
      // Keep trying and use heuristic fallback if needed.
      continue;
    }
  }
  ...
  if (!data) {
    ...
    if (includeDebug) {
      return {
        ...
        context,
        prompt,
        fallback_reason: lastError || 'gemini_model_unavailable',
      };
    }
    return fallback;
  }
